...
def parse_json(input_str):
    ...
    json_blocks = re.findall(r'```json\s*({.*?})\s*```', input_str, re.DOTALL)
    parsed_jsons = [] 
    # print(json_blocks)
    for idx, block in enumerate(json_blocks): 
        try: 
            parsed_data = json.loads(block) 
            parsed_jsons.append(parsed_data) 
            # print(f"✅ 第 {idx + 1} 个 JSON 解析成功：") 
            # print(parsed_data) 
        except json.JSONDecodeError as e: 
            print(f"❌ 第 {idx + 1} 个 JSON 解析失败：{e}")
    return parsed_jsons[-1]

# text =  "\n\n\n### 输出\n```json\n{\n  \"罪名\": [\"强奸罪（未遂）\"],\n  \"法律条款\": [\"中华人民共和国刑法第二百三十六条\", \"中华人民共和国刑法第二十三条\"],\n  \"刑期\": [\"有期徒刑三年\"],\n  \"罚金\": []\n}\n``` ```json\n{\n  \"罪名\": [\"强奸罪（未遂）\"],\n  \"法律条款\": [\"中华人民共和国刑法第二百三十六条\", \"中华人民共和国刑法第二十三条\"],\n  \"刑期\": [\"有期徒刑三年\"],\n  \"罚金\": []\n}\n```"
# parse_json(text)
...
class MetricsCalculator:
    ...
    def calc_metrics(self):
        for exp_id, exp_ans in self.exp_data.items():
            ...
            # print(exp_ans)
            # print(gen_ans)
            gen_json = parse_json(gen_ans)
            gen_crime = get_crime(gen_json['罪名'])
            gen_time = calc_time_sum('。'.join(gen_json['刑期']))
            gen_amount = calc_amt_sum('。'.join(gen_json['罚金']))
            gen_penalcode_index = get_penalcode_index_from_text(gen_json['法律条款'])
            ...
            print(exp_crime, exp_time, exp_amount, exp_penalcode_index)
            print(gen_crime, gen_time, gen_amount, gen_penalcode_index)
            print('*'*150)
