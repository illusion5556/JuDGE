...
def get_penalcode_ljp(doc): # 这里是一个简化了的处理方法，匹配所有“第xxx条”并转换成阿拉伯数字
...
class MetricsCalculator:
    ...
    def get_all_from_text(self, text): 
        crime = get_crime_ljp(parse_crime(text))
        prison = calc_time_sum_ljp('。'.join(parse_prison(text)))
        fine = calc_amt_sum('。'.join(parse_fine(text)))
        laws = get_penalcode_ljp(parse_laws(text))
    ...
    def get_all_from_text_exp(self, text):
    ...
    def calc_metrics(self):
        for exp_id, exp_ans in self.exp_data.items():
            ...
            exp_crime, exp_time, exp_amount, exp_penalcode_index = self.get_all_from_text_exp(exp_ans)
